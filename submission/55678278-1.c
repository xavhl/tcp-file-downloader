...
int main() {
	...
	while (1) {
		memset(file_name, 0, sizeof(file_name)); //empty file_name
...
int writeFile(int sockfd, FILE* fp) {
	...
	u_long iMode = 1;//otherwise recv() will cause infinite while loop and wait for server input even if server side buffer is empty
	ioctlsocket(sockfd, FIONBIO, &iMode);//switching will allow while loop to skip this condition and continue if recv() is blocking
